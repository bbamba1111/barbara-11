...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Check if we should open the audit immediately (from popup)
    if (typeof window !== "undefined") {
      const shouldOpenAudit = sessionStorage.getItem("openAuditImmediately")
      if (shouldOpenAudit === "true") {
        // Clear the flag
        sessionStorage.removeItem("openAuditImmediately")
        // Open the audit
        setShowAudit(true)
      }
    }
  ...
  // Function to open the audit (used by SitePopup)
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-4 md:p-24 bg-gradient-to-b from-white to-rose-50">
      {/* Add the site popup with the openAudit callback */}
      <SitePopup onTakeAudit={handleOpenAudit} />
